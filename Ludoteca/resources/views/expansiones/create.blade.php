...
@section('content')
    <div class="mx-auto max-w-2xl rounded-3xl border border-slate-800 bg-slate-900/70 p-8 shadow-2xl shadow-slate-950/50">
        ...
        <form action="{{ route('expansiones.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <select id="juego_id" name="juego_id" required class="w-full rounded-xl border border-slate-700 bg-slate-950 px-4 py-3 text-white outline-none transition focus:border-cyan-400">
                    ...
                    @foreach($juegos as $juego)
                        <option value="{{ $juego->id }}" {{ old('juego_id') == $juego->id ? 'selected' : '' }}>{{ $juego->titulo }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('expansiones.index') }}" class="rounded-xl border border-slate-700 bg-slate-800 px-5 py-3 font-medium text-slate-200 transition hover:border-slate-500">Cancelar</a>
            </div>
        </form>
    </div>
@endsection
