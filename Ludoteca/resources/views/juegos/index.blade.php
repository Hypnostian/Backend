...
@section('content')
    <div class="mb-8 flex items-center justify-between gap-4">
        <div>
            ...
            <h1 class="mt-2 text-3xl font-bold text-white">Juegos</h1>
        </div>
        <a href="{{ route('juegos.create') }}" class="rounded-xl bg-cyan-500 px-4 py-2.5 font-semibold text-slate-950 transition hover:bg-cyan-400">+ Nuevo juego</a>
    </div>
        ...
        @forelse($juegos as $juego)
            <div class="rounded-3xl border border-slate-800 bg-slate-900/70 p-6 shadow-lg shadow-slate-950/40 transition hover:-translate-y-1 hover:border-cyan-400/40">
                ...
                <div class="mt-6 flex gap-3">
                    ...
                    <a href="{{ route('juegos.edit', $juego->id) }}" class="rounded-xl border border-slate-700 px-3 py-2 text-sm font-medium text-slate-200 transition hover:border-slate-500">Editar</a>
                </div>
            </div>
        @empty
